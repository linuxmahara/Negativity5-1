...
// Negativity: optional setting to unlock wallet for block minting only;
...
// Negativity: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Negativity: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // Negativity: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // Negativity: not avatar mode
                    {
...
// Negativity: create coin stake transaction
...
// Negativity: check 'spent' consistency between wallet and txindex
// Negativity: fix wallet spent state according to txindex
...
// Negativity: disable transaction (only for coinstake)
