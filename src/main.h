...
static const int LAST_POW_BLOCK = 101000;
...
static const int STAKE_TARGET_SPACING = 16;
...
public:
    ...
    {
        // Negativity: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    int64 GetMinFee(unsigned int nBlockSize=1, bool fAllowFree=false, enum GetMinFee_mode mode=GMF_BLOCK) const
    {
        ...
        if (fAllowFree)
        {
            if (nBlockSize == 1)
            {
                // Transactions under Negativity are free
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Negativity: get transaction coin age
...
public:
    ...
    // Negativity: block signature - signed by coin base txout[0]'s owner
    ...
    // Negativity: two types of block: proof-of-work or proof-of-stake
    ...
    // Negativity: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Negativity: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Negativity: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Negativity: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Negativity: block index flags
    enum  
    {
